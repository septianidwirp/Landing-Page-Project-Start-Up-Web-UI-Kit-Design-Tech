        ...
        if (!username.value == '') {
            if (username.value.match(min5) && username.value.match(max10)) {
                if (!username.value.match(space) && !username.value.match(specialCharacters)) {
                    ...
                    usernameError.innerHTML = "Can not contain space and special characters";
                }
            } else {
                usernameError.innerHTML = 'Must be between 5 and 10 characters';
            }
        } else {
                        ...
                        if(password.value.match(min6) && password.value.match(max12)){
                            if(!password.value.match(space) && !password.value.match(specialCharacters)){
                                if(password.value == passwordValidation.value) {
                                    ...
                                    passwordError.innerHTML = "Does not match";
                                    passwordValidationError.innerHTML = "Does not match";
                                ...
                                passwordError.innerHTML = "Can not contain space and special characters";
                            }
                        } else {
                            passwordError.innerHTML = "Must be between 6 and 12 characters";
                        }
                    } else {
                        passwordError.innerHTML = "Must contain at least one number";
                    }
                } else {
        ...
        if (usernameCheck){
            if (emailCheck){
                if (passwordCheck){
                    ...
                    errorText.innerHTML = "Check your password";
                    const toast = new bootstrap.Toast(errorToast);
                    toast.show();
                }
            } else {
                errorText.innerHTML = "Check your email";
                const toast = new bootstrap.Toast(errorToast);
                toast.show();
            }
        } else {
            errorText.innerHTML = "Check your username";
            const toast = new bootstrap.Toast(errorToast);
            toast.show();
        }
    }
...
if (url.indexOf("login.html") > -1) {
    ...
    function login() {
        if (localStorage.getItem('user') == null) {
            errorText.innerHTML = "Rlease register first!";
            const toast = new bootstrap.Toast(errorToast);
            toast.show();
